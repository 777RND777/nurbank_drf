...
def test_create_application(admin_client, value):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    response = admin_client.post('/applications/', {'alue': settings.MAX_APPLICATION_VALUE + 1, 'user': user_id})
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    response = admin_client.post('/applications/', {'value': value, 'user': 0})
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    response = admin_client.post('/applications/', {'value': value, 'ser': user_id})
